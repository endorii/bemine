...
import { CircleCheckIcon, CircleXIcon, EyeIcon, EyeOffIcon } from "lucide-react";
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3">
            <div className="flex flex-col gap-0.5">
                <label className="text-sm text-zinc-600 font-semibold">E-mail</label>
                <input
                    type="text"
            ...
            <div className="flex flex-col gap-0.5">
                <label className="text-sm text-zinc-600 font-semibold">Phone</label>
                <input
                    type="text"
                    className={`p-3 rounded bg-blue-600/5 outline-0 border-b-2 border-transparent focus:border-blue-600 focus:rounded-b-none ${
                    ...
                    {...register("phone")}
                />{" "}
                <div className="text-red-600 text-sm mt-1">{errors.phone?.message}</div>
            </div>
            <div className="flex flex-col gap-0.5">
                <label className="text-sm text-zinc-600 font-semibold">Password</label>
                ...
                <div className="mt-2 text-sm">
                    <div className="mb-1 text-zinc-700">Your password should contain:</div>

                    <div className="flex flex-col gap-0.5">
                        <CheckItem valid={passwordChecks.length} label="9 or more symbols" />
                        <CheckItem valid={passwordChecks.uppercase} label="Uppercase letter" />
                        <CheckItem valid={passwordChecks.lowercase} label="Lowercase letter" />
                        <CheckItem valid={passwordChecks.number} label="Minimum 1 number" />
                    </div>
                </div>
            </div>
            ...
            <div className="flex flex-col gap-2 mt-4">
                <div className="text-sm text-zinc-700">
                    ...
                    <Link href={"#"} className="font-semibold hover:underline hover:text-blue-600">
                        Terms of Use
                    </Link>
                </div>
                ...
                <div className="flex gap-3 items-center text-xs text-zinc-700">
                    ...
                    <div>Yes, I want to receive information about Bemine news and promotions.</div>
                </div>
            </div>

            <button type="submit" className="p-3 bg-blue-600 text-white rounded-md font-medium">
                Sign Up
            </button>
        </form>
    );
...
const CheckItem = ({ valid, label }: { valid: boolean; label: string }) => (
    <div className="flex items-center gap-2">
        ...
        <span className={valid ? "text-blue-600" : "text-red-600"}>{label}</span>
    </div>
);
